...
opts_knit$set(root.dir ="~/Documents/Spring Rotation/Variant-Caller-Pipeline/data/vcf/")
args <- commandArgs(trailingOnly = TRUE)

args <- c("03_RET7a.txt.Q40.vcf.gz","03_RET7.txt.Q40.vcf.gz","03_RET7b.txt.Q40.vcf.gz")
pairs <- list()
...
get_con_matrix <- function(f1, f2) {
  ...
  r$Comparison <- sprintf('%s__%s', sub(".txt","",sub(".vcf.gz","",f1)), sub(".txt","",sub(".vcf.gz","",f2)))
  r
}
...
get_pair_results <- function(f1, f2) {
  ...
  comp <- sprintf('%s__%s', sub(".txt","",sub(".vcf.gz","",f1)), sub(".txt","",sub(".vcf.gz","",f2)))
  id_repl <- list('0'=f1, '1'=f2, '2'=comp)
  
  # Replace ID Field as is appropriate

  SN$id <- gsub(".vcf.gz","",gsub(".txt", "", as.character(id_repl[as.character(SN$id)])))
  ...
  id_rev <- list()
  id_rev[unlist(id_repl)] <- names(id_repl)
  SN$orig_id <- as.character(id_rev[as.character(SN$id)])
  
  # Set SNP counts to actual, instead of subtraction
  #SN$orig_id <- SN$orig_id[]
...
# Remove duplicates
snp_indel <- as.data.frame(snp_indel[!duplicated(snp_indel$id),])
snp_indel[snp_indel$orig_id == "NULL", "orig_id"] <- "Unique (within comparison)"
snp_indel[snp_indel$orig_id == 2, "orig_id"] <- "Union"
...
plots <- lapply(names(p_list), function(i) {
  ggplot(snp_indel) + 
    geom_bar(stat="identity", aes_string(x="id", y=p_list[[i]])) +
    geom_text( aes_string(x="id",y=p_list[[i]], label=p_list[[i]], vjust=-1)) +
    theme(legend.position="bottom", legend.title=element_blank()) +
    labs(y=i, x="Call Set",title=i) +
    scale_y_continuous(breaks=number_ticks(10), labels=scientific_format()) + 
    facet_grid(.~ orig_id, space = "free", scales="free", drop=T )
})
```
...
# Total SNPs
ggplot(PSC_data) + 
    geom_line(aes(x=sample, y= nonRef_snps, group=id, color=id)) +
    facet(id ~ .)
