...
dir <- "/Users/daniel/Documents/git/Variant-Caller-Pipeline/data/vcf/"
...
#args <- sprintf("../../data/vcf/%s", c("03_RET1b.txt.Q40.vcf.gz","03_RET1.txt.Q40.vcf.gz","03_RET1a.txt.Q40.vcf.gz"))
...
get_con_matrix <- function(f1, f2) {
  ...
  r <- read.csv2(pipe(sprintf("echo %s | xargs -I {} -n 1 -P 6 sh -c \"bcftools gtcheck -S {} -g %s %s | sed \'s/$/\t{}/\'\" | egrep -v '#' ", paste0(joint_samples, collapse=" "), f1, f2)), as.is=T, sep='\t',header=F, col.names= c("Concordance", "Uncertainty", "Average Depth", "Number of Sites", "Sample", "Query"))
...
opts_chunk$set(fig.width=12, fig.height=8, fig.path=paste0('../reports/',paste0(repl_multiple(args, replace_words),collapse="_"),"/"),
...
plots <- lapply(names(p_list), function(i) {
  ggplot(snp_indel) + 
    ...
    geom_text( aes_string(x="name",y=p_list[[i]], label=p_list[[i]], vjust=-1)) +
...
# Total SNPs
ggplot(PSC_data) + 
    geom_point(aes(x=sample, y= nonRef_snps, group=id, color=id)) +
...
# Singletons
ggplot(PSC_data) + 
    geom_line(aes(x=sample, y=nSingletons, group=id, color=id)) +
    geom_point(aes(x=sample, y=nSingletons, group=id, color=id)) +
    labs(title="SNP count/file", x="Strain Name", y="non-reference Allele") +
    theme(legend.position="top", axis.text.x = element_text(angle = 90, hjust = 1))
...
con_chart <- function(record) {
  ...
  # Restrict to strains in common.
  record <- filter(record, Query %in% Sample & Sample %in% Query)
