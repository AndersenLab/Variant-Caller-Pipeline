...
args <- c("02a_BGI2_rep1.txt.Q40.vcf.gz", "02b_BGI2_rep2.txt.Q40.vcf.gz")
pairs <- list()
...
get_pair_results <- function(f1, f2) {
  ...
  tmp <- tempfile()
  system(sprintf('bcftools gtcheck -G l -s %s %s > %s', f1, f2, tmp))
...
multi_allelic <- ggplot(snp_indel[snp_indel$variable=="number.of.indels." ,], 
                aes(x=filename, y=value, fill=filename)) +
  geom_bar(stat="identity") +
  geom_text( aes(label=value,y=value, vjust=-1)) +
  facet_grid(. ~ variable) +
