...
get_concordance_matrix <- function(f1, f2) {
  ...
  r <- read.csv2(pipe(sprintf("echo %s | xargs -t -I {} -n 1 -P 5 sh -c \"bcftools gtcheck  -s {} -G 1 -g %s %s | sed \'s/$/\t{}/\'\" | egrep -v '#' ", paste0(joint_samples[1:20], collapse=" "), f1, f2)), as.is=T, sep='\t',header=F, col.names= c("CN","Discordance_total", "Discordance_per_site", "Number_of_Sites", "Sample", "Sample_ID", "Query"))
...
concordance_chart <- function(record, union=F) {
  ...
  ggsave(filename = paste0(results_dir, plot_title, ".png", collapse=""), height=12, width=12)
}
...
lapply(names(p_list), function(i) {
  ggplot(snp_indel) + 
    ...
    ggsave(filename = paste0(results_dir,p_list[[i]], ".png", collapse=""), plot=last_plot())
})
...
ggplot(PSC_data) + 
    ...
    ggsave(filename = paste0(results_dir, "Total_SNPs.png", collapse=""))
...
ggplot(PSC_data) + 
    ...
    ggsave(filename = paste0(results_dir,"Singletons_SNPs.png", collapse="")
...
ggsave(filename = paste0(results_dir, "individual_concordance.png"))
...
## Save Data!
save(list = ls(all = TRUE), file=paste0(results_dir,make.names(paste0(args,collapse="")),".Rdata",collapse=""))
