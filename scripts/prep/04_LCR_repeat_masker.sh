...
## LCR 1
cd ../../data/LCR_Region/
curl 'http://hgdownload.soe.ucsc.edu/goldenPath/ce10/database/rmsk.txt.gz' | gunzip -kfc - | grep 'Low_complexity' | cut -f 6,7,8 | awk '{ print $0 "\tLCR_Region"}' | bedtools merge -i -  > ce10.ucsc.masked.bed
...
curl 'ftp://ftp.wormbase.org/pub/wormbase/releases/WS220/species/c_elegans/c_elegans.WS220.genomic_masked.fa.gz' > c_elegans.WS220.genomic_masked.fa.gz
