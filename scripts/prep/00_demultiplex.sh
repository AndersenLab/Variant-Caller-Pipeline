...
paste -d '\0' <(gunzip -kfc $a2) <(awk '{ if (NR%2+1==1) { print } else { print "" } }'  $aindex) | gzip > Princeton_read1.fq.gz 
...
for r in `ls *.fq`; do
    # Trim Barcodes
    seqtk trimfq -e 8 $r | gzip > ${r%%.fq}.trimmed.fq.gz
    # Generate quality reports
    fastqc ${r%%.fq}.trimmed.fq.gz
done
